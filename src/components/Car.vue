  ...
  <div class="car">
    ...
    <div class="carmain">
    	...
    	<div class="marsk" v-show="confirm">
    		<div class="que">
    			<p>确认删除？</p>
    			<button style="margin-right: 5px;" @click="conf">确定</button><button @click="cancel">取消</button>
    		</div>
    	</div>
    </div>
  </div>
...
import Checkboxr from "@/components/Checkboxr"
...
export default {
  ...
  methods:{
  	...
  	conf(){
    	delete this.items["id"+this.items[this.index].id];
    	this.confirm=false;
			//传入修改后的数组
			this.xiu(this.items);
			//重新计算价格
